!(function() {
	...
	var addInitHook = function() {
		var showOnHover =
			this.options.measurementOptions && this.options.measurementOptions.showOnHover
	...
	L.Polyline.include({
		...
		updateMeasurements: function() {
			...
			if (this._measurementOptions.showDistances && latLngs.length > 1) {
				formatter =
					this._measurementOptions.formatDistance || L.bind(this.formatDistance, this)
	...
	L.Circle.include({
		...
		updateMeasurements: function() {
			...
			if (options.showArea) {
				...
				L.marker
					.measurement(latLng, formatter(area), options.lang.totalArea, 0, options)
					.addTo(this._measurementLayer)
			}
		},
	})
